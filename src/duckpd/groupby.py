...
class DataFrameGroupBy:
    ...
    def __getitem__(self, key: str | Sequence[str]) -> SeriesGroupBy | DataFrameGroupBy:
        ...
        all_labels = list(self._by) + [label for label in labels if label not in self._by]
        ...
        **kwargs: object,
        ...
        if isinstance(func, str):
            ...
            if not named_kwargs and func_name != "size":
                raise UnsupportedOperationError(f"No valid columns for aggregation {func!r}")
        ...
        if isinstance(func, dict):
            ...
            for col_name_obj, agg_spec in dict_map.items():
                if not isinstance(col_name_obj, str):
                    raise TypeError("Dictionary aggregation keys must be column name strings")
                if isinstance(agg_spec, str):
                    named_kwargs[col_name_obj] = (col_name_obj, agg_spec)
                else:
                    raise UnsupportedOperationError(
                        "Currently only single string function per column is supported in dict agg"
                    )
            return self._named_agg(**named_kwargs)

        raise UnsupportedOperationError(f"Unsupported agg argument type: {type(func).__name__}")
    ...
    def _convenience_agg(self, func_name: str, *, numeric_only: bool) -> DataFrame:
        ...
        for col in data_cols:
            if numeric_only and not is_numeric_type(col.duckdb_type):
                continue
            if func_name in {"sum", "mean", "min", "max"} and not is_numeric_type(col.duckdb_type):
                if numeric_only:
                    continue
            ...
            named_kwargs[col.label] = (col.label, func_name)
        if not named_kwargs:
            raise UnsupportedOperationError(f"No numeric columns available for {func_name}")
        return self._named_agg(**named_kwargs)
        ...
        **kwargs: object,
        ...
        for output_name, raw_spec in kwargs.items():
            if not isinstance(raw_spec, tuple):
                msg = (
                    f"Named aggregation for {output_name!r} must be a (column_name, agg_func) tuple"
                )
                raise TypeError(msg)
            ...
            if not isinstance(target_col_name, str):
                raise TypeError(f"Target column name in {output_name!r} must be a string")
...
class SeriesGroupBy:
        ...
        if isinstance(by, str):
            ...
            if by._session is not series._session or by._plan is not series._plan:
                raise AlignmentError(
                    "Grouping Series from a different frame requires explicit index alignment"
                )
            ...
            raise TypeError("by must be a string, Series, or sequence of strings/Series")
    ...
    def rolling(
        ...
        *,
        ...
        keys = tuple(find_column(frame._plan.metadata, label) for label in self._df_groupby._by)
        return GroupedRolling(
            target,
            keys,
            window,
    ...
    def agg(self, func: object) -> DataFrame:
        ...
        target_name = self._series.name if self._series.name is not None else "__duckpd_val__"
        if self._series.name is None:
            self._df_groupby._frame = self._df_groupby._frame.assign(__duckpd_val__=self._series)
