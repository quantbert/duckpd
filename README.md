...
* **🔍 Typed Semantic Retrieval:** Stream text through an optional pinned local ONNX model, or encode ordered time-series windows with deterministic DuckDB expressions; persist verified `FLOAT[n]` vectors or run exact transient top-k without pandas materialization.
