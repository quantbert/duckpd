...
| **`duckpd.FeatureStore(source, ...)`** / **`session.feature_store(...)`** | `FeatureStore` | *(Not available)* | Creates an embedded feature store connecting to local or remote (`hf://`, `http://`, or `https://`) Parquet datasets. The production contract uses one atomic UTC-day file at `year=YYYY/month=MM/day=DD/part.parquet`; legacy yearly and monthly catalog templates remain readable. Supports exact multi-family equi-joins and point-in-time (`ASOF LEFT JOIN`) alignment with `availability_delay` lookahead protection and optional catalog-declared `history_lookback` partition bounds. Returns lazy `duckpd.DataFrame` objects. |
...
| `duckpd.series_representation(...)` / `duckpd.series_embedding_model(...)` / `duckpd.tspulse_series_embedding_model(...)` / `duckpd.ts2vec_series_embedding_model(...)` | **`[DuckPD Extension]`** | Immutable window, channel, sampling, data-contract, normalization, ordered learned channel roles, optional learned-encoder declarations, one pinned TSPulse contract, and one attested local TS2Vec bundle | `SeriesRepresentationSpec` / `SeriesEmbeddingModelSpec` | Builds canonical representation identities. Planning performs no model preparation or inference. Native, explicitly prepared custom providers, optional CPU-only TSPulse, and optional CPU-only TS2Vec execute through `DataFrame.embed_series()` and exact `DataFrame.vector.search_series()`. Both built-ins are experimental controls, not qualified financial models. |
...
a compatible PyTorch build and `transformers`; DuckPD never replaces that
