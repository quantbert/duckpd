...
| **`duckpd.FeatureStore(source, ...)`** / **`session.feature_store(...)`** | `FeatureStore` | *(Not available)* | Creates an embedded feature store connecting to local or remote (`hf://`) Parquet feature datasets. Supports exact multi-family equi-joins and point-in-time (`ASOF LEFT JOIN`) alignment with `availability_delay` lookahead bias protection. Returns lazy `duckpd.DataFrame` objects. |
