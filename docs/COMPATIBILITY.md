...
| `duckpd.series_representation(...)` / `duckpd.series_embedding_model(...)` | **`[DuckPD Extension]`** | Immutable window, channel, sampling, data-contract, normalization, ordered learned channel roles, and optional learned-encoder declarations | `SeriesRepresentationSpec` / `SeriesEmbeddingModelSpec` | Builds canonical, side-effect-free representation identities. Planning performs no model preparation or inference. Native and explicitly prepared custom learned representations execute through `DataFrame.embed_series()` and exact `DataFrame.vector.search_series()`; no built-in learned adapter is qualified. |
